...
import type { AssignmentAggregate, AssignmentSummary, BudgetStatus, ConditionalRoute, ResourceBudget, ResourceUsage, RetryPolicy, RetryState, UsageObservation, WorkflowEvent, WorkflowRepository } from "./types.ts";
...
function eventPayload(event: WorkflowEvent): Record<string, JsonValue> {
  ...
  const templateApplicationId = event.type.startsWith("template.") ? data.templateApplicationId : undefined;
  const recordId = String(templateApplicationId ?? data.routeId ?? data.planId ?? data.approvalId ?? data.decisionId ?? data.evidenceId ?? data.taskId ?? event.assignmentId);
  const recordType = templateApplicationId ? "template-application" : data.routeId ? "route" : data.planId ? "plan" : data.approvalId ? "approval" : data.decisionId ? "decision" : data.evidenceId ? "evidence" : data.taskId ? "task" : "assignment";
...
export class StateWorkflowRepository implements WorkflowRepository {
  ...
  async get(assignmentId: string): Promise<AssignmentAggregate | undefined> {
    ...
    const [plansRaw, approvalsRaw, tasksRaw, routesRaw, templateApplicationsRaw, executionsRaw, actionsRaw, resultsRaw, assessmentsRaw, decisionsRaw, evidenceRaw, checkpointsRaw, taskPacketsRaw, usageRaw, lease] = await Promise.all([
      records("plans"), records("approvals"), records("tasks"), records("routes"), records("template-applications"), records("executions"), records("actions"), records("results"), records("assessments"), records("decisions"),
      records("evidence"), records("checkpoints"), records("task-packets"), records("usage"), readOwnershipLease(stateRoot, assignmentId),
    ]);
    ...
    const aggregate: AssignmentAggregate = {
      ...
      startedAt: String(assignment.started_at ?? assignment.created_at), ...(assignment.budget ? { budget: assignment.budget as ResourceBudget } : {}), ...(assignment.budget_status ? { budgetStatus: assignment.budget_status as unknown as BudgetStatus } : {}), usage: (assignment.usage as unknown as ResourceUsage | undefined) ?? { inputTokens: 0, outputTokens: 0, cachedInputTokens: 0, costMicros: 0 },
      usageObservations: usageRaw.map((item) => ({ observationId: String(item.observation_id), assignmentId: String(item.assignment_id), taskId: String(item.task_id), executionId: String(item.execution_id), provider: String(item.provider), model: String(item.model), measurement: item.measurement as "observed" | "estimated", inputTokens: Number(item.input_tokens), outputTokens: Number(item.output_tokens), cachedInputTokens: Number(item.cached_input_tokens ?? 0), costMicros: Number(item.cost_micros), observedAt: String(item.observed_at) } satisfies UsageObservation)),
      currentPlan, plans,
      ...
      tasks: tasksRaw.map((item, index) => ({ taskId: String(item.task_id), state: item.state as never, ...(item.role ? { role: String(item.role) } : {}), affectedPaths: item.affected_paths as string[], executionIds: item.execution_ids as string[], resultIds: item.result_ids as string[], dependencies: (item.dependencies as Array<Record<string, unknown>> | undefined)?.map(dependency => ({ taskId: String(dependency.task_id), required: dependency.required !== false, acceptableStates: (dependency.acceptable_states as Array<"accepted" | "closed"> | undefined) ?? ["accepted", "closed"] })) ?? [], gates: (item.gates as Array<Record<string, unknown>> | undefined)?.map(gate => ({ gateId: String(gate.gate_id), name: String(gate.name), phase: gate.phase as "pre_dispatch" | "pre_acceptance", required: gate.required !== false, status: gate.status as "pending" | "passed" | "failed" | "waived", evidenceIds: (gate.evidence_ids as string[] | undefined) ?? [], ...(gate.updated_at ? { updatedAt: String(gate.updated_at) } : {}) })) ?? [], ...(item.run_budget ? { runBudget: item.run_budget as ResourceBudget } : {}), ...(item.retry_policy ? { retryPolicy: item.retry_policy as RetryPolicy } : {}), ...(item.retry_state ? { retryState: item.retry_state as unknown as RetryState } : {}), priority: Number(item.priority ?? 0), createdSequence: Number(item.created_sequence ?? index + 1) })),
      routes: routesRaw.map((item) => ({ routeId: String(item.route_id), sourceTaskId: String(item.source_task_id), sourceKind: item.source_kind as ConditionalRoute["sourceKind"], ...(item.gate_id ? { gateId: String(item.gate_id) } : {}), expectedValue: String(item.expected_value), targetTaskId: String(item.target_task_id), effect: item.effect as ConditionalRoute["effect"], state: item.state as ConditionalRoute["state"], createdAt: String(item.created_at), ...(item.evaluated_at ? { evaluatedAt: String(item.evaluated_at) } : {}), ...(item.observed_value ? { observedValue: String(item.observed_value) } : {}) })),
      templateApplications: templateApplicationsRaw.map((item) => ({ applicationId: String(item.application_id), templateId: String(item.template_id), templateVersion: String(item.template_version), templateDigest: String(item.template_digest), templateSnapshot: item.template_snapshot as never, bindings: item.bindings as never, operationBindings: item.operation_bindings as Record<string, string>, routeBindings: item.route_bindings as Record<string, string>, createdAt: String(item.created_at) })),
      executions: executionsRaw.map((item) => ({ executionId: String(item.execution_id), taskId: String(item.task_id), attempt: Number(item.attempt), ...(item.run_kind ? { runKind: item.run_kind as "initial" | "retry" | "rework" } : {}), ...(item.retry_attempt ? { retryAttempt: Number(item.retry_attempt) } : {}), state: item.state as never, ...(item.started_at ? { startedAt: String(item.started_at) } : {}), ...(item.ended_at ? { endedAt: String(item.ended_at) } : {}), ...(item.budget ? { budget: item.budget as ResourceBudget } : {}), ...(item.budget_status ? { budgetStatus: item.budget_status as unknown as BudgetStatus } : {}), usage: (item.usage as unknown as ResourceUsage | undefined) ?? { inputTokens: 0, outputTokens: 0, cachedInputTokens: 0, costMicros: 0 } })),
  ...
  async save(aggregate: AssignmentAggregate): Promise<void> {
    ...
    const assignment: StoredRecord = {
      ...
      ...(aggregate.startedAt ? { started_at: aggregate.startedAt } : {}), ...(aggregate.budget ? { budget: aggregate.budget as unknown as JsonValue } : {}), ...(aggregate.budgetStatus ? { budget_status: aggregate.budgetStatus as unknown as JsonValue } : {}), ...(aggregate.usage ? { usage: aggregate.usage as unknown as JsonValue } : {}),
    ...
    for (const task of aggregate.tasks) {
      const record: StoredRecord = { schema_version: "1.0", record_id: task.taskId, record_type: "task", created_at: await createdAtFor(task.taskId), updated_at: now, task_id: task.taskId, assignment_id: aggregate.assignmentId, state: task.state, ...(task.role ? { role: task.role } : {}), affected_paths: task.affectedPaths, execution_ids: task.executionIds, result_ids: task.resultIds, dependencies: (task.dependencies ?? []).map(dependency => ({ task_id: dependency.taskId, required: dependency.required, acceptable_states: dependency.acceptableStates })), gates: (task.gates ?? []).map(gate => ({ gate_id: gate.gateId, name: gate.name, phase: gate.phase, required: gate.required, status: gate.status, evidence_ids: gate.evidenceIds, ...(gate.updatedAt ? { updated_at: gate.updatedAt } : {}) })), ...(task.runBudget ? { run_budget: task.runBudget as unknown as JsonValue } : {}), ...(task.retryPolicy ? { retry_policy: task.retryPolicy as unknown as JsonValue } : {}), ...(task.retryState ? { retry_state: task.retryState as unknown as JsonValue } : {}), priority: task.priority ?? 0, created_sequence: task.createdSequence ?? aggregate.tasks.findIndex(candidate => candidate.taskId === task.taskId) + 1 };
    ...
    for (const route of aggregate.routes ?? []) {
      const record: StoredRecord = { schema_version: "1.0", record_id: route.routeId, record_type: "route", created_at: route.createdAt, updated_at: now, route_id: route.routeId, assignment_id: aggregate.assignmentId, source_task_id: route.sourceTaskId, source_kind: route.sourceKind, ...(route.gateId ? { gate_id: route.gateId } : {}), expected_value: route.expectedValue, target_task_id: route.targetTaskId, effect: route.effect, state: route.state, ...(route.evaluatedAt ? { evaluated_at: route.evaluatedAt } : {}), ...(route.observedValue ? { observed_value: route.observedValue } : {}) };
      await writeAssignmentRecord({ stateRoot, assignmentId: aggregate.assignmentId, category: "routes", record, immutable: false, validator, schemaId: "route" });
    }
    for (const application of aggregate.templateApplications ?? []) {
      const record: StoredRecord = { schema_version: "1.0", record_id: application.applicationId, record_type: "template-application", created_at: application.createdAt, application_id: application.applicationId, assignment_id: aggregate.assignmentId, template_id: application.templateId, template_version: application.templateVersion, template_digest: application.templateDigest, template_snapshot: application.templateSnapshot as unknown as JsonValue, bindings: application.bindings as unknown as JsonValue, operation_bindings: application.operationBindings, route_bindings: application.routeBindings };
      await writeAssignmentRecord({ stateRoot, assignmentId: aggregate.assignmentId, category: "template-applications", record, immutable: true, validator, schemaId: "template-application" });
    }
    for (const execution of aggregate.executions) {
      const record: StoredRecord = { schema_version: "1.0", record_id: execution.executionId, record_type: "execution", created_at: await createdAtFor(execution.executionId), updated_at: now, execution_id: execution.executionId, task_id: execution.taskId, assignment_id: aggregate.assignmentId, attempt: execution.attempt, ...(execution.runKind ? { run_kind: execution.runKind } : {}), ...(execution.retryAttempt ? { retry_attempt: execution.retryAttempt } : {}), state: execution.state, ...(execution.startedAt ? { started_at: execution.startedAt } : {}), ...(execution.endedAt ? { ended_at: execution.endedAt } : {}), ...(execution.budget ? { budget: execution.budget as unknown as JsonValue } : {}), ...(execution.budgetStatus ? { budget_status: execution.budgetStatus as unknown as JsonValue } : {}), ...(execution.usage ? { usage: execution.usage as unknown as JsonValue } : {}) };
    ...
    for (const observation of aggregate.usageObservations ?? []) {
      const record: StoredRecord = { schema_version: "1.0", record_id: observation.observationId, record_type: "usage-observation", created_at: observation.observedAt, observation_id: observation.observationId, assignment_id: observation.assignmentId, task_id: observation.taskId, execution_id: observation.executionId, provider: observation.provider, model: observation.model, measurement: observation.measurement, observed_at: observation.observedAt, input_tokens: observation.inputTokens, output_tokens: observation.outputTokens, cached_input_tokens: observation.cachedInputTokens, cost_micros: observation.costMicros };
      await writeAssignmentRecord({ stateRoot, assignmentId: aggregate.assignmentId, category: "usage", record, immutable: true, validator, schemaId: "usage-observation" });
    }
